<!--
  Canonical copy of the main-api repo's agent instruction file.

  In real use this content lives at the root of the plaza-main-api repository —
  as CLAUDE.md (Claude Code), .github/copilot-instructions.md (Copilot), or
  AGENTS.md (Codex). This copy is the canon: edit here, then copy into the repo
  (WORKFLOW.md Step 5f). Never edit the repo's copy directly.
...
2. If the task is system-wide (auth, multi-tenancy, cross-service flow), also read
   `00-architecture-overview.md`; for a naming or convention question,
   `CONVENTIONS.md` alone is enough.
